...
@login_required
def dash_root_manage_acheteur_scoring_rating(request, acheteur_id):
    acheteur = get_object_or_404(
    ...
    try:
        ...
        logger.error(f"Erreur JWT scoring rating: {e}")
        messages.error(request, "Erreur d'authentification. Veuillez vous reconnecter.")
        return redirect_to_login(request.get_full_path())
    ...
    return render(request, "main/root/acheteur/scoring/dash_root_manage_acheteur_scoring_rating.html", context)
...
def dash_root_manage_acheteur_emailling_test(request, acheteur_id):
    ...
    for i in range(nombre):
        ...
        commande = Commande.objects.create(
            ...
            email=acheteur_choisi.emails.values_list('email', flat=True).first() or '',
            pays=pays,
            ville=ville,
    ...
    for actif in actifs_qs:
        ...
        try:
            ...
            years_data.append({
                ...
                'frno':           _fmt(r.fonds_de_roulement_normatif, decimals=1, unit='j'),
                'autonomie_fin':  _fmt(r.autonomie_fin,  unit='%'),
                'auto_raw':       float(r.autonomie_fin  or 0),
                # Liquidité
                ...
                'rent_eco':       _fmt(r.rentabilite_economique, unit='%'),
                'rent_eco_raw':   float(r.rentabilite_economique or 0),
                'rent_fin':       _fmt(r.rentabilite_fin,         unit='%'),
                'rent_fin_raw':   float(r.rentabilite_fin         or 0),
                'rop':            _fmt(r.rentabilite_de_loutil_de_production, unit='%'),
                ...
                # Solvabilité
                'solvabilite':    _fmt(r.solvabilite, unit='%'),
                'solv_raw':       float(r.solvabilite or 0),
                'roe':            _fmt(r.rendement_capitaux_propres, unit='%'),
                'roe_raw':        float(r.rendement_capitaux_propres or 0),
