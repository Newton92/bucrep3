...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write(self.style.SUCCESS('Successfully imported Domaines et Postes data!'))
    ...
    def clear_existing_data(self, skip_domaines, skip_postes):
        ...
        if not skip_postes:
            self.stdout.write(self.style.WARNING('Clearing existing PosteEntreprise data...'))
            postes_count = PosteEntreprise.objects.count()
            PosteEntreprise.objects.all().delete()
            self.stdout.write(self.style.SUCCESS(f'Cleared {postes_count} postes.'))
        
        if not skip_domaines:
            # Les domaines doivent être supprimés après les postes (dépendances)
            self.stdout.write(self.style.WARNING('Clearing existing DomaineEntreprise data...'))
            domaines_count = DomaineEntreprise.objects.count()
            DomaineEntreprise.objects.all().delete()
            self.stdout.write(self.style.SUCCESS(f'Cleared {domaines_count} domaines.'))
    ...
    def import_data(self, post_total_data, liste_nouvelles_fonctions, skip_domaines, skip_postes):
            ...
            for i, (domaine_libelle, postes) in enumerate(post_total_data, 1):
                try:
                    ...
                    if created:
                        ...
                        self.stdout.write(self.style.SUCCESS(f'  ✓ Domaine créé: {code} - {libelle_minuscule}'))
                    else:
                        stats['domaines_updated'] += 1
                        self.stdout.write(self.style.WARNING(f'  ↻ Domaine mis à jour: {code} - {libelle_minuscule}'))
        ...
        if not skip_postes:
            ...
            for domaine_libelle, postes in post_total_data:
                ...
                if not domaine:
                    self.stdout.write(self.style.ERROR(f"  Domaine {domaine_code} not found for postes"))
                    continue
                    ...
                    try:
                        ...
                        if created:
                            ...
                            if poste_counter % 50 == 0 or poste_counter <= 10:
                                self.stdout.write(f'    ✓ Poste créé: {code} - {libelle_minuscule}')
                        else:
                            stats['postes_updated'] += 1
                            if poste_counter % 100 == 0:
                                self.stdout.write(f'    ↻ Poste mis à jour: {code}')
                ...
                for code, libelle, domaine_parent_code in liste_nouvelles_fonctions:
                    try:
                        ...
                        if not domaine_parent:
                            self.stdout.write(self.style.WARNING(f"  Domaine parent {domaine_parent_code} not found for {code}"))
                            continue
                        ...
                        if created:
                            stats['postes_created'] += 1
                            self.stdout.write(self.style.SUCCESS(f'  ✓ Nouvelle fonction créée: {code} - {libelle}'))
                        else:
                            stats['postes_updated'] += 1
                            self.stdout.write(self.style.WARNING(f'  ↻ Nouvelle fonction mise à jour: {code} - {libelle}'))
    ...
    def simulate_import(self, post_total_data, liste_nouvelles_fonctions, clear_data, skip_domaines, skip_postes):
        ...
        if clear_data:
            if not skip_postes:
                self.stdout.write(self.style.WARNING("Would clear all PosteEntreprise data"))
            if not skip_domaines:
                self.stdout.write(self.style.WARNING("Would clear all DomaineEntreprise data"))
        ...
        if self.verify_source_data(post_total_data):
            self.stdout.write(self.style.SUCCESS("✓ No duplicates found in source data"))
        else:
            self.stdout.write(self.style.WARNING("⚠ Possible duplicates in source data"))
    ...
    def verify_database_integrity(self):
        ...
        if postes_sans_domaine > 0:
            self.stdout.write(self.style.ERROR(f"Postes sans domaine: {postes_sans_domaine}"))
        else:
            self.stdout.write(self.style.SUCCESS("✓ Tous les postes ont un domaine"))
        
        if domaines_sans_postes > 0:
            self.stdout.write(self.style.WARNING(f"Domaines sans postes: {domaines_sans_postes}"))
        ...
        if codes_duplicates:
            self.stdout.write(self.style.ERROR(f"Codes en double: {codes_duplicates}"))
        else:
            self.stdout.write(self.style.SUCCESS("✓ Tous les codes sont uniques"))
    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write(f"✓ {len(created_domaines)} domaines créés")
                ...
                self.stdout.write(f"✓ {len(created_postes)} postes créés")
