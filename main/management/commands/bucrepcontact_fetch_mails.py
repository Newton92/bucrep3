...
def extract_data_from_email(body):
    ...
    texte_apres_priorite = (
        re.split(r"Priority\s*:\s*\w+", body, maxsplit=1)[-1].strip()
        if priorite_match
        else ""
    )
    ...
    return {
        ...
        "internal_bp_id": (
            internal_bp_id_match.group(1) if internal_bp_id_match else None
        ),
        "nom": nom_match.group(1).strip() if nom_match else None,
        "identifiants": (
            identifiants_match.group(1).strip() if identifiants_match else None
        ),
...
def fetch_new_bucrepcontact_emails():
        ...
        for response_part in msg_data:
            if isinstance(response_part, tuple):
                ...
                if date_tuple:
                    ...
                    date_reception = datetime.fromtimestamp(
                        timestamp, pytz.UTC
                    )  # Conversion UTC
                ...
                if msg.is_multipart():
                    for part in msg.walk():
                        if part.get_content_type() == "text/plain":
                            ...
                            data["date_reception"] = (
                                date_reception  # Ajout de la date de réception
                            )

                            if data[
                                "reference"
                            ]:  # Vérifier qu'on a bien extrait des données
                                CredendoCommande.objects.create(email_id=num, **data)
                                print(
                                    f"Commande Credendo enregistrée avec date {date_reception} pour email ID: {num}"
                                )
