...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if keep_recent:
            ...
            self.stdout.write(f"[STATS] Données actuelles:")
        ...
        if not confirm:
            confirm = input("[FAIL] Voulez-vous vraiment tout supprimer? (yes/NO): ")
            if confirm.lower() != 'yes':
                self.stdout.write("[WARNING] " + '[FAIL] Suppression annulée')
                return
        ...
        with transaction.atomic():
            ...
            self.stdout.write('[DONE] Pièces jointes supprimées')
            
            # 2. Supprimer les infos de mail
            MailInfo.objects.all().delete()
            self.stdout.write('[DONE] Historique emails supprimé')
            
            # 3. Supprimer les suivis
            SuiviCommande.objects.all().delete()
            self.stdout.write('[DONE] Suivis supprimés')
            ...
            self.stdout.write('[DONE] Commandes supprimées')
