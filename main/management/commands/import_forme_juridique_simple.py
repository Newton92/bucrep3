...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if duplicates:
            self.stdout.write(self.style.ERROR(f"WARNING: Duplicate codes in source data: {duplicates}"))
            if not dry_run:
            ...
            try:
                ...
                if existing:
                    ...
                    if data_identical:
                        ...
                        updated_count += 1
                        if i % 5 == 0 or i <= 10:
                            self.stdout.write(f"  ↻ Updated: {code} - {libelle}")
                else:
...
class CommandSimple(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i, (code, libelle, poids, active, description) in enumerate(formes_juridiques, 1):
            ...
            if created:
                self.stdout.write(f"{i:3}. ✓ Created: {code} - {libelle}")
            else:
                self.stdout.write(f"{i:3}. ↻ Updated: {code} - {libelle}")
