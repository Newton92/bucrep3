...
class Command(BaseCommand):
        ...
        for code, libelle, poids in entries:
            ...
            if existing:
                ...
                if existing.libelle == libelle_str and existing.poids == poids:
                    ...
                    updated_count += 1
                    self.stdout.write(
                        self.style.WARNING(f'[UPD] Updated: {code} - {libelle_str} (Poids: {poids})')
                    )
            else:
                ...
                created_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f'[OK] Created: {code} - {libelle_str} (Poids: {poids})')
                )
    ...
    def print_stats(self, stats):
        ...
        if total_processed == stats['total']:
            ...
            self.stdout.write(
                self.style.ERROR(f"[WARN] Mismatch: processed {total_processed} out of {stats['total']}")
            )
