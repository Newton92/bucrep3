...
class Resume(models.Model):
    ...
    devise = models.ForeignKey('Devise', null=True,blank=True, on_delete=models.DO_NOTHING, verbose_name="Dévise capital social", related_name="devise_capital_social")
    capital_social = models.DecimalField(max_digits=100, decimal_places=0, blank=True, null=True)
    chiffre_affaire = models.DecimalField(max_digits=100, decimal_places=0, blank=True, null=True)
    resultat_net = models.DecimalField(max_digits=100, decimal_places=0, blank=True, null=True)
    capitaux_propre =models.DecimalField(max_digits=100, decimal_places=0, blank=True, null=True)
    nombre_employe = models.DecimalField(max_digits=100, decimal_places=0, blank=True, null=True)
    date_creation = models.DateField(null=True, blank=True)
    couleur_commentaire = models.ForeignKey('CouleurCommentaire', null=True,blank=True,on_delete=models.DO_NOTHING)
    commentaire = models.TextField(blank=True, max_length=10000000)
    

class DonneesEnregistrement(models.Model):
    
    acheteur = models.ForeignKey('Acheteur', null=True, on_delete=models.DO_NOTHING, blank=True)
    date_creation = models.DateField(blank=True, null=True)
    date_registre = models.DateField(blank=True, null=True)
    forme_juridique =  models.CharField(max_length=4000, choices=FORMEJURIDIQUE_CHOICES, default="Veuillez choisir la forme juridique")
    ...
    statut_registre = models.CharField(max_length=4000,choices=LIEN_STATUT_CHOICE,default="--------")
...
class Tendance(models.Model):
    ...
    acheteur = models.ForeignKey('Acheteur', null=True, on_delete=models.DO_NOTHING)
    avis_commercial  = models.CharField(max_length=100, choices=LIEN_AVIS_COMMERCIAL_CHOICE, blank=True)
    ...
    plus_informations  = models.CharField(max_length=100, choices=LIEN_PLUS_INFORMATIONS_NOTATION_CHOICE, blank=True, verbose_name="Plus d'informations sur la notation ")
    ...
    presse_media = models.CharField(max_length=100, blank=True, verbose_name="Presse/Medias")
    ...
    principaux_concurrent = models.TextField(blank=True, max_length=10000000, verbose_name="Principaux concurrents")
    commentaire = models.TextField(max_length=10000000, blank=True)
...
class ResponsableAcheteur(models.Model):
    ...
    Sexe = models.CharField(max_length=20, default=STATUS_MASCULIN, choices=STATUS_CHOICES, blank=True, null=True)
    poste =  models.CharField(max_length=100,choices=BON_POST_CHOICES_CHOICES, blank=True)
...
class AntecedantsJuridique(models.Model):
    ...
    acheteur = models.ForeignKey('Acheteur', blank=True, null=True, on_delete=models.DO_NOTHING)
...
class RiskManagment(models.Model):
    ...
    acheteur = models.ForeignKey('Acheteur', null=True, on_delete=models.DO_NOTHING)
    professionalisme = models.CharField(max_length=20, default=STATUS_AUCUN, choices=STATUS_CHOICES, verbose_name="Professionalisme")
    organisation = models.CharField(max_length=20, default=STATUS_AUCUN, choices=STATUS_CHOICES)
    turn_over = models.CharField(max_length=20, default=STATUS_AUCUN, choices=STATUS_CHOICES, verbose_name="Non départ des employés" )
    greve = models.CharField(max_length=20, default=STATUS_AUCUN, choices=STATUS_CHOICES, verbose_name="Non grève")
    degradation_qualite = models.CharField(max_length=20, default=STATUS_AUCUN, choices=STATUS_CHOICES, verbose_name="Non dégradation de la qualité du travail")
    non_respect_condition = models.CharField(max_length=20, default=STATUS_AUCUN, choices=STATUS_CHOICES, verbose_name="Respect des Employés")
    couleur_commentaire = models.ForeignKey('CouleurCommentaire', null=True,blank=True,on_delete=models.DO_NOTHING)
    commentaire = models.TextField(blank=True, max_length=10000000)
...
class ConseilAdministration(models.Model):
    ...
    fonction_dans_le_conseil =  models.CharField(max_length=100, choices=BON_POST_CHOICES_CHOICES, blank=True)
    numero_adresse = models.CharField(max_length=200, blank=True, verbose_name='Numéro adresse')
    rue_adresse = models.CharField(max_length=200, blank=True, verbose_name='Rue adresse')
    code_postale_adresse = models.CharField(max_length=200, blank=True, verbose_name="Code postal adresse")
    couleur_commentaire = models.ForeignKey('CouleurCommentaire', null=True, blank=True,on_delete=models.DO_NOTHING)
    commentaire = models.TextField(blank=True, max_length=10000000)
...
class CompositionCapitalSocial(models.Model):
    ...
    acheteur = models.ForeignKey('Acheteur', null=True, blank=True, on_delete=models.DO_NOTHING)
    devise = models.ForeignKey('Devise', null=True, blank=True, on_delete=models.DO_NOTHING, verbose_name="Dévise capital libéré")
    emis = models.DecimalField(max_digits=100, decimal_places=5, blank=True, null=True)
    publie = models.DecimalField(max_digits=100, decimal_places=5, blank=True, null=True)
    libere = models.DecimalField(max_digits=100, decimal_places=5, blank=True, null=True)
    couleur_commentaire = models.ForeignKey('CouleurCommentaire', null=True, blank=True,on_delete=models.DO_NOTHING)
    commentaire = models.TextField(blank=True, max_length=10000000)
...
class CompositionAction(models.Model):
    ...
    pourcentage = models.DecimalField(max_digits=100, decimal_places=5, blank=True, null=True)
    couleur_commentaire = models.ForeignKey('CouleurCommentaire', null=True,
...
class Structure(models.Model):
    ...
    acheteur = models.ForeignKey('Acheteur', null=True, blank=True, on_delete=models.DO_NOTHING)
    nom = models.CharField(blank=True, max_length=200)
    type_affiliation =  models.CharField(max_length=100,choices=LIEN_ENTREPRISE_CHOICE, blank=True)
    numero_adresse = models.CharField(max_length=200, blank=True, verbose_name='Numéro adresse')
    rue_adresse = models.CharField(max_length=200, blank=True, verbose_name='Rue adresse')
    code_postale_adresse = models.CharField(max_length=200, blank=True, verbose_name="Code postal adresse")
    couleur_commentaire = models.ForeignKey('CouleurCommentaire', null=True, blank=True,on_delete=models.DO_NOTHING)
...
class CompteFinancier(models.Model):
    ...
    credibilite_cabinet= models.CharField(blank=True, max_length=200, choices=STATUS__OUI_NON, verbose_name="Crédibilité du cabinet pour ACREMAC")
    source = models.CharField(blank=True, max_length=200)
    presentation = models.CharField(blank=True, max_length=200)
    date_compte = models.DateField(blank=True, verbose_name="Début de période de compte N")
    date_fin = models.DateField(blank=True, null=True, verbose_name="Fin clôture de compte N")
    date_compte_n_moins_un = models.DateField(blank=True,null=True, verbose_name="Début de période de compte N-1")
    date_fin_n_moins_un = models.DateField(blank=True,null=True, verbose_name="Fin clôture de compte N-1")
    date_compte_n_moins_deux = models.DateField(blank=True,null=True, verbose_name="Début de période de compte N-2")
    date_fin_n_moins_deux = models.DateField(blank=True,null=True, verbose_name="Fin clôture de compte N-2")
    type_compte = models.CharField(blank=True, max_length=200, null=True)
    devise = models.CharField(max_length=20, default=XAF, choices=STATUS_CHANGE, blank=True)
    type_bilan = models.CharField(max_length=255,choices=LIEN_TYPE_BILAN_CHOICE,default="--------")
    couleur_commentaire = models.ForeignKey('CouleurCommentaire', null=True,blank=True,on_delete=models.DO_NOTHING)
    commentaire = models.TextField(blank=True, max_length=10000000)
...
class ProprieteEtActif(models.Model):
    ...
    acheteur = models.ForeignKey('Acheteur', null=True, blank=True, on_delete=models.DO_NOTHING)
    locaux =  models.CharField(max_length=255,choices=LIEN_COMPORTEMENT_PREMISES_CHOICE, blank=True)
...
class ConditionDeVente(models.Model):
    ...
    acheteur = models.ForeignKey('Acheteur', null=True, blank=True, on_delete=models.DO_NOTHING)
    local = models.CharField(max_length=255, blank=True)
    recouvrement_de_dette_jugement = models.CharField(max_length=255, choices=LIEN_COMPORTEMENT_JUGEMENT_CHOICE, default="--------")
    ...
    comportement_de_paiement = models.CharField(max_length=255, choices=LIEN_COMPORTEMENT_PAIEMENT_CHOICE, default="--------")
